...
const MEAL_OPTIONS = [
  "Oats breakfast with fruits & nuts",
  "Grilled salmon, sweet potato & spinach",
  "Lentil stew with mixed salad (low-sodium)",
  "Quinoa bowl with roasted veg",
  "Chickpea salad with lemon dressing",
  "Steamed fish with steamed greens"
];
...
export default function Hypertension() {
  ...
  const storageKey = "user_profile_hypertension";
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("Could not parse hypertension profile:", e);
    }
  ...
  const handleCalculatePlan = () => {
    ...
    const workout = planWorkout(profile, "hypertension") || { items: [] };
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-4xl mx-auto">
        <header className="mb-6">
          ...
          <p className="text-gray-600 mt-2">Enter a few profile details, pick low-sodium & heart-healthy meals, then create a tailored plan.</p>
        </header>
          ...
          <section id="mealSelection" className="bg-white p-6 rounded-2xl shadow-sm mb-6">
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-sm text-gray-600">Pick low-sodium options — selected meals will be prioritized in the plan.</p>
              </div>
              <div>
                ...
                return (
                  <label key={m} className={`flex items-center gap-3 p-3 rounded-lg border ${checked ? "border-indigo-500 bg-indigo-50" : "border-gray-200 bg-white"}`}>
                    ...
                    <div>
                      ...
                      <div className="text-sm text-gray-500">Heart-healthy & low-sodium</div>
                    </div>
                  </label>
                );
              })}
