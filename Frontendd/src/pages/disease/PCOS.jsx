...
const MEAL_OPTIONS = [
  ...
  "Egg & spinach scramble with wholegrain toast",
  "Mixed bean salad with olive oil dressing",
  "Roasted chicken with roasted vegetables (small portion)"
];

const WORKOUT_OPTIONS = [
  { name: "Resistance training", notes: "3x/week focusing on big muscle groups" },
  { name: "Short HIIT", notes: "10–15 minute intervals if tolerated" },
  { name: "Brisk walk", notes: "30–40 minutes daily" }
];
...
export default function PCOS() {
  ...
  const [step, setStep] = useState(1); // 1 profile, 2 meal select, 3 result
  const navigate = useNavigate();
  const storageKey = "user_profile_pcos";
  ...
  useEffect(() => {
    ...
    // Prefill disease-specific profile if previously saved
    try {
      ...
      console.warn("Could not parse PCOS profile:", e);
    }
  ...
  const onProfileSubmit = (values) => {
    // values: { name, age, height, weight, gender, mealCount? }
    setProfile(values);
    setStep(2);
    setSelectedMeals([]);
    setResult(null);
  ...
  const handleCalculatePlan = () => {
    ...
    // planMeals & planWorkout should accept profile and disease key
    const mealPlan = planMeals(profile, "pcos") || {};
    const workout = planWorkout(profile, "pcos") || { items: WORKOUT_OPTIONS };
    ...
    try {
      localStorage.setItem("myfit_last_plan", JSON.stringify({ disease: "pcos", tdee: combined.tdee, meals: combined.meals }));
    } catch (_) {}
  ...
  const handleSave = (res) => {
    try {
      const saved = { disease: "pcos", profile, timestamp: Date.now(), ...res };
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-4xl mx-auto">
        <header className="mb-6">
          <h1 className="text-3xl font-bold">PCOS — Personalized Planner</h1>
          ...
          <section id="mealSelection" className="bg-white p-6 rounded-2xl shadow-sm mb-6">
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-sm text-gray-600">Pick meals that improve insulin sensitivity and satiety.</p>
              </div>
              <div>
                ...
                return (
                  <label key={m} className={`flex items-center gap-3 p-3 rounded-lg border ${checked ? "border-indigo-500 bg-indigo-50" : "border-gray-200 bg-white"}`}>
                    ...
                    <div>
                      ...
                      <div className="text-sm text-gray-500">PCOS-friendly option</div>
                    </div>
                  </label>
                );
              })}
