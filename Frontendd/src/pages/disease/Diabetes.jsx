...
const MEAL_OPTIONS = [
  ...
  "Grilled chicken with spinach & brown rice",
  "Lentil soup and mixed salad",
  "Quinoa bowl with beans & veggies",
  "Vegetable stir-fry with tofu",
  "Chia seed pudding with berries (small portion)"
];
...
export default function Diabetes() {
  ...
  const storageKey = "user_profile_diabetes";
  ...
  useEffect(() => {
    ...
    // prefill from localStorage if present (keeps this disease profile separate)
    try {
      ...
      console.warn("Could not parse diabetes profile:", e);
    }
  ...
  const handleCalculatePlan = () => {
    ...
    const workout = planWorkout(profile, "diabetes") || { items: [] };
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-4xl mx-auto">
        <header className="mb-6">
          ...
          <p className="text-gray-600 mt-2">Enter a profile, choose low-GI meals, then create a glucose-sensible plan.</p>
        </header>
          ...
          <section id="mealSelection" className="bg-white p-6 rounded-2xl shadow-sm mb-6">
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-sm text-gray-600">Select low-glycemic meals to prioritize in your plan.</p>
              </div>
              <div>
                ...
                return (
                  <label key={m} className={`flex items-center gap-3 p-3 rounded-lg border ${checked ? "border-indigo-500 bg-indigo-50" : "border-gray-200 bg-white"}`}>
                    ...
                    <div>
                      ...
                      <div className="text-sm text-gray-500">Low-GI option</div>
                    </div>
                  </label>
                );
              })}
