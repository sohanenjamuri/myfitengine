...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    const unsub = onAuthStateChanged(auth, async (u) => {
      ...
      // try to fetch Firestore doc users/{uid}
      try {
        const ref = doc(db, "users", u.uid);
        const snap = await getDoc(ref);
        if (snap.exists()) {
          const data = snap.data();
          // Fill fields if present in Firestore
  ...
  const handleSave = async (ev) => {
    ...
    try {
      ...
      try {
        ...
        // continue — we'll still write Firestore doc
      }

      // write to Firestore users/{uid} (merge)
...
// export default function Profile() {
//   const { loading, profile, saveProfile, refresh } = useUserProfile();
//   const [saving, setSaving] = useState(false);
//   const navigate = useNavigate();

//   useEffect(() => {
//     // nothing extra here; hook already refreshes on auth change
//   }, [refresh]);
...
//     setSaving(false);
//     if (res.ok) {
//       // saved successfully -> go back to dashboard
...
//           <p className="text-gray-600 mt-2">Enter or update your details — saving will return you to the dashboard.</p>
//         </header>

//         <ProfileForm
//           initial={profile || {}}
//           onSubmit={onSubmit}
//           submitLabel={saving ? "Saving…" : "Save profile & go to Dashboard"}
//         />
//       </div>
//     </div>
//   );
